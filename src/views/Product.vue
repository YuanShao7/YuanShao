<template>
  <div class="container" :style="{'background-image':`url(/${items.limg})`}">   
   <div class="top_bar"><!--顶部三个按钮-->
           ...
           <p>{{items.ltitle}}</p>
            <div class="flap">            
              <p>
                ...
                <span class="text">{{items.lcoupon}}</span>
              </p>
              <p>
                <img src="/img/jinb.png" alt="" class="size">
                <span class="text">{{items.lgold}}</span>
              </p>
              <p>                                
                <span class="text">¥ {{product.lrmb.toFixed(2)}}</span>
              </p>            
            </div>
            <div class="todate_1">
              <span class="topdate">上架时间 ：{{items.ldate}}</span>
    ...
    <div class="bottom_bar">
      ...
      <van-button color="linear-gradient(to right, #f90, #f70)" size="large" :data-lid="items.lid" :data-dianquan="items.lcoupon" :data-jinbi="items.lgold" :data-price="items.lrmb" :data-img="items.limg"  :data-cname="items.ltitle" @click="addshoppingcart">加入购物车</van-button>
...
export default {
  ...
  data(){
    return{
      ...
      product:{lrmb:0},
      pics:[],
      // bgi:{backgroundImage:url(`items.limg`)}
    }
  },
  computed:{
  ...
  mounted(){
  this.axios.get('/shop/all_details', {
            params: {
                lid:this.lid,
            }
        }).then(result=>{
      this.items=result.data[0];
      this.product.lrmb=this.items.lrmb;
