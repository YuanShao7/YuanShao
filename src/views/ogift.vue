<template>
  <div class="container" :style="{'background-image':`url(/${items.oimg})`}">   
   <div class="top_bar"><!--顶部三个按钮-->
           ...
           <p>{{items.otitle}}</p>
            <div class="flap">            
              <p>
                ...
                <span class="text">{{items.ocoupon}}</span>
              </p>            
              <p>        
                <span class="text">¥ {{product.ormb.toFixed(2)}}</span>
              </p>            
            </div>
            <div class="todate_1">
              <span class="topdate">上架时间 ：{{items.odate}}</span>
    ...
    <div class="bottom_bar">
      ...
      <van-button color="linear-gradient(to right, #f90, #f70)" size="large" :data-lid="items.oid" :data-dianquan="items.ocoupon" :data-jinbi="0" :data-price="items.ormb" :data-img="items.oimg"  :data-cname="items.otitle" @click="addshoppingcart">加入购物车</van-button>
...
export default {
 ...
 mounted(){
   this.axios.get('/shop/all_ogift', {
      ...
      this.product.ormb=this.items.ormb;
