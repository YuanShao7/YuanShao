<template>
  <div class="container" :style="{'background-image':`url(/${items.qimg})`}">   
   <div class="top_bar"><!--顶部三个按钮-->
           ...
           <p>{{items.qtitle}}</p>
            <div class="flap">            
              <p>
                ...
                <span class="text">{{items.qcoupon}}</span>
              </p>            
              <p>        
                <span class="text">¥ {{product.qrmb.toFixed(2)}}</span>
              </p>            
            </div>
            <div class="todate_1">
              <span class="topdate">上架时间 ：{{items.qdate}}</span>
    ...
    <div class="bottom_bar">
      ...
      <van-button color="linear-gradient(to right, #f90, #f70)" size="large" :data-lid="items.qid" :data-dianquan="items.qcoupon" :data-jinbi="0" :data-price="items.qrmb" :data-img="items.qimg"  :data-cname="items.qtitle" @click="addshoppingcart">加入购物车</van-button>
...
export default {
  ...
  methods:{
   ...
   this.axios.get('/shop/all_pifu', {
            params: {
                lid:this.lid
            }
        }).then(result=>{
      this.items=result.data[0];
      this.product.qrmb=this.items.qrmb;
